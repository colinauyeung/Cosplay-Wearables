...
#     while True :
#         ax, ay, az, gx, gy, gz, temperature = mpu.readAllData()
#         print( "ax:{:0.2f} ay:{:0.2f} az:{:0.2f}".format( ax, ay, az ) )
# #         print( "gx:{:0.2f} gy:{:0.2f} gz:{:0.2f}".format( gx, gy, gz ) )
#         time.sleep( 0.01 )



#     # test the sample rate that we can realize
    # nTest = 50000
    # t1 = time.time_ns()
    # for i in range(nTest) :
    #     ax, ay, az, gx, gy, gz, temp = mpu.readAllData()
    # t2 = time.time_ns()
    # period = (t2-t1) * 1e-9 / nTest
    # print( "sample period(s):", period )
    # print( "sample rate(Hz):", 1/period )
    ...
    with open("test10.csv", mode="w") as file:
        ...
        for i in range(500):
            ...
            # writer.writerow([euler[0], euler[1], euler2[0], euler2[1], i])
            print( 1 / (t2 - t1) )
            t1 = t2
